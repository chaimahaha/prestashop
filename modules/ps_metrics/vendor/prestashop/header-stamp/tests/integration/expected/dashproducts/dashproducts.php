...
class dashproducts extends \Module
{
    ...
    public function getTableRecentOrders()
    {
        ...
        foreach ($orders as $order) {
            ...
            $tr = array();
            $tr[] = array('id' => 'firstname_lastname', 'value' => '<a href="' . $this->context->link->getAdminLink('AdminCustomers', \true) . '&id_customer=' . $order['id_customer'] . '&viewcustomer">' . \Tools::htmlentitiesUTF8($order['firstname']) . ' ' . \Tools::htmlentitiesUTF8($order['lastname']) . '</a>', 'class' => 'text-left');
            $tr[] = array('id' => 'total_products', 'value' => \count(OrderDetail::getList((int) $order['id_order'])), 'class' => 'text-center');
            $tr[] = array('id' => 'total_paid', 'value' => \Tools::displayPrice((float) $order['total_paid_tax_excl'], $currency), 'class' => 'text-center', 'wrapper_start' => $order['valid'] ? '<span class="badge badge-success">' : '', 'wrapper_end' => '<span>');
            $tr[] = array('id' => 'date_add', 'value' => \Tools::displayDate($order['date_add']), 'class' => 'text-center');
            $tr[] = array('id' => 'status', 'value' => \Tools::htmlentitiesUTF8($order['state_name']), 'class' => 'text-center');
            $tr[] = array('id' => 'details', 'value' => '', 'class' => 'text-right', 'wrapper_start' => '<a class="btn btn-default" href="index.php?tab=AdminOrders&id_order=' . (int) $order['id_order'] . '&vieworder&token=' . \Tools::getAdminTokenLite('AdminOrders') . '" title="' . $this->trans('Details', array(), 'Modules.Dashproducts.Admin') . '"><i class="icon-search"></i>', 'wrapper_end' => '</a>');
            $body[] = $tr;
        }
    ...
    public function getTableMostViewed($date_from, $date_to)
    {
        ...
        if (\Configuration::get('PS_STATSDATA_PAGESVIEWS')) {
            ...
            if (\is_array($products) && \count($products)) {
                foreach ($products as $product) {
                    $product_obj = new \Product((int) $product['id_object'], \true, $this->context->language->id);
                    if (!\Validate::isLoadedObject($product_obj)) {
                        continue;
                    }
                    $img = '';
    ...
    {
        return array('DASHPRODUCT_NBR_SHOW_LAST_ORDER' => \Configuration::get('DASHPRODUCT_NBR_SHOW_LAST_ORDER'), 'DASHPRODUCT_NBR_SHOW_BEST_SELLER' => \Configuration::get('DASHPRODUCT_NBR_SHOW_BEST_SELLER'), 'DASHPRODUCT_NBR_SHOW_MOST_VIEWED' => \Configuration::get('DASHPRODUCT_NBR_SHOW_MOST_VIEWED'), 'DASHPRODUCT_NBR_SHOW_TOP_SEARCH' => \Configuration::get('DASHPRODUCT_NBR_SHOW_TOP_SEARCH'));
    }
