...
class Gsitemap extends \Module
{
    ...
    public function getContent()
    {
        ...
        $this->context->smarty->assign(array('gsitemap_form' => './index.php?tab=AdminModules&configure=gsitemap&token=' . \Tools::getAdminTokenLite('AdminModules') . '&tab_module=' . $this->tab . '&module_name=gsitemap', 'gsitemap_cron' => $store_url . 'modules/gsitemap/gsitemap-cron.php?token=' . \Tools::substr(\Tools::encrypt('gsitemap/cron'), 0, 10) . '&id_shop=' . $this->context->shop->id, 'gsitemap_feed_exists' => \file_exists($this->normalizeDirectory(\_PS_ROOT_DIR_) . 'index_sitemap.xml'), 'gsitemap_last_export' => \Configuration::get('GSITEMAP_LAST_EXPORT'), 'gsitemap_frequency' => \Configuration::get('GSITEMAP_FREQUENCY'), 'gsitemap_store_url' => $store_url, 'gsitemap_links' => \Db::getInstance()->ExecuteS('SELECT * FROM `' . \_DB_PREFIX_ . 'gsitemap_sitemap` WHERE id_shop = ' . (int) $this->context->shop->id), 'store_metas' => $store_metas, 'gsitemap_disable_metas' => \explode(',', \Configuration::get('GSITEMAP_DISABLE_LINKS')), 'gsitemap_customer_limit' => array('max_exec_time' => (int) \ini_get('max_execution_time'), 'memory_limit' => (int) \ini_get('memory_limit')), 'prestashop_ssl' => \Configuration::get('PS_SSL_ENABLED'), 'gsitemap_check_image_file' => \Configuration::get('GSITEMAP_CHECK_IMAGE_FILE'), 'shop' => $this->context->shop));
    ...
    public function addLinkToSitemap(&$link_sitemap, $new_link, $lang, &$index, &$i, $id_obj)
    {
        if ($i <= 25000 && \memory_get_usage() < 100000000) {
            ...
            if ($index % 20 == 0 && !$this->cron) {
                ...
                \header('Refresh: 5; url=http' . (\Configuration::get('PS_SSL_ENABLED') ? 's' : '') . '://' . \Tools::getShopDomain(\false, \true) . \__PS_BASE_URI__ . 'modules/gsitemap/gsitemap-cron.php?continue=1&token=' . \Tools::substr(\Tools::encrypt('gsitemap/cron'), 0, 10) . '&type=' . $new_link['type'] . '&lang=' . $lang . '&index=' . $index . '&id=' . (int) $id_obj . '&id_shop=' . $this->context->shop->id);
                die;
            } else {
                if ($this->cron) {
                    ...
                    \Tools::redirectAdmin($this->context->link->getAdminLink('AdminModules', \true, array(), array('tab_module' => $this->tab, 'module_name' => $this->name, 'continue' => 1, 'type' => $new_link['type'], 'lang' => $lang, 'index' => $index, 'id' => (int) $id_obj, 'id_shop' => $this->context->shop->id)));
                }
                die;
            }
        }
    ...
    protected function getProductLink(&$link_sitemap, $lang, &$index, &$i, $id_product = 0)
    {
        ...
        foreach ($products_id as $product_id) {
            ...
            $file_headers = \Configuration::get('GSITEMAP_CHECK_IMAGE_FILE') ? @\get_headers($image_link) : \true;
            $image_product = array();
    ...
    protected function getCategoryLink(&$link_sitemap, $lang, &$index, &$i, $id_category = 0)
    {
        ...
        foreach ($categories_id as $category_id) {
            ...
            if (isset($image_link) && ($file_headers[0] != 'HTTP/1.1 404 Not Found' || $file_headers === \true)) {
                $image_category = array('title_img' => \htmlspecialchars(\strip_tags($category->name)), 'caption' => \Tools::substr(\htmlspecialchars(\strip_tags($category->description)), 0, 350), 'link' => $image_link);
            }
    ...
    {
        $modules_links = \Hook::exec(self::HOOK_ADD_URLS, array('lang' => $lang), null, \true);
    ...
    public function createSitemap($id_shop = 0)
    {
        ...
        $languages = \Language::getLanguages(\true, $this->context->shop->id);
        $lang_stop = \Tools::getValue('lang') ? \true : \false;
    ...
    {
        \fwrite($fd, '<loc>' . (\Configuration::get('PS_REWRITING_SETTINGS') ? '<![CDATA[' . $loc . ']]>' : $loc) . '</loc>' . \PHP_EOL . ($last_mod ? '<lastmod>' . \date('c', \strtotime($last_mod)) . '</lastmod>' : '') . \PHP_EOL . '<changefreq>' . $change_freq . '</changefreq>' . \PHP_EOL . '<priority>' . \number_format($priority, 1, '.', '') . '</priority>' . \PHP_EOL);
    }
