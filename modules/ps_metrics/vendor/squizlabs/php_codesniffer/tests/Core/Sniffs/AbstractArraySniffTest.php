 ...
 */
namespace ps_metrics_module_v4_0_6\PHP_CodeSniffer\Tests\Core\Sniffs;
...
final class AbstractArraySniffTest extends AbstractMethodUnitTest
{
    ...
    public function testSimpleValues()
    {
        ...
        $expected = [0 => ['value_start' => $token + 1], 1 => ['value_start' => $token + 3], 2 => ['value_start' => $token + 5]];
    ...
    public function testSimpleKeyValues()
    {
        ...
        self::$sniff->process(self::$phpcsFile, $token);
        $expected = [0 => ['index_start' => $token + 1, 'index_end' => $token + 1, 'arrow' => $token + 2, 'value_start' => $token + 3], 1 => ['index_start' => $token + 5, 'index_end' => $token + 5, 'arrow' => $token + 6, 'value_start' => $token + 7], 2 => ['index_start' => $token + 9, 'index_end' => $token + 9, 'arrow' => $token + 10, 'value_start' => $token + 11]];
        $this->assertSame($expected, self::$sniff->indicies);
    }
    ...
    public function testMissingKeys()
    {
        ...
        self::$sniff->process(self::$phpcsFile, $token);
        $expected = [0 => ['index_start' => $token + 1, 'index_end' => $token + 1, 'arrow' => $token + 2, 'value_start' => $token + 3], 1 => ['value_start' => $token + 5], 2 => ['index_start' => $token + 7, 'index_end' => $token + 7, 'arrow' => $token + 8, 'value_start' => $token + 9]];
        $this->assertSame($expected, self::$sniff->indicies);
    }
    //end testMissingKeys()
    /**
    ...
    public function testMultiTokenKeys()
    {
        ...
        self::$sniff->process(self::$phpcsFile, $token);
        $expected = [0 => ['index_start' => $token + 4, 'index_end' => $token + 8, 'arrow' => $token + 10, 'value_start' => $token + 12], 1 => ['index_start' => $token + 16, 'index_end' => $token + 20, 'arrow' => $token + 22, 'value_start' => $token + 24]];
        $this->assertSame($expected, self::$sniff->indicies);
    }
    ...
    public function testMissingKeysCoalesceTernary()
    {
        ...
        self::$sniff->process(self::$phpcsFile, $token);
        $expected = [0 => ['index_start' => $token + 3, 'index_end' => $token + 3, 'arrow' => $token + 5, 'value_start' => $token + 7], 1 => ['value_start' => $token + 31], 2 => ['value_start' => $token + 39]];
        $this->assertSame($expected, self::$sniff->indicies);
    }
    ...
    public function testTernaryValues()
    {
        ...
        self::$sniff->process(self::$phpcsFile, $token);
        $expected = [0 => ['index_start' => $token + 3, 'index_end' => $token + 3, 'arrow' => $token + 5, 'value_start' => $token + 7], 1 => ['index_start' => $token + 32, 'index_end' => $token + 32, 'arrow' => $token + 34, 'value_start' => $token + 36], 2 => ['index_start' => $token + 72, 'index_end' => $token + 72, 'arrow' => $token + 74, 'value_start' => $token + 76]];
        $this->assertSame($expected, self::$sniff->indicies);
    }
    ...
    public function testHeredocValues()
    {
        ...
        self::$sniff->process(self::$phpcsFile, $token);
        $expected = [0 => ['value_start' => $token + 4], 1 => ['value_start' => $token + 10]];
        $this->assertSame($expected, self::$sniff->indicies);
    }
    ...
    public function testArrowFunctionValue()
    {
        ...
        self::$sniff->process(self::$phpcsFile, $token);
        $expected = [0 => ['index_start' => $token + 4, 'index_end' => $token + 4, 'arrow' => $token + 6, 'value_start' => $token + 8], 1 => ['index_start' => $token + 12, 'index_end' => $token + 12, 'arrow' => $token + 14, 'value_start' => $token + 16], 2 => ['index_start' => $token + 34, 'index_end' => $token + 34, 'arrow' => $token + 36, 'value_start' => $token + 38]];
        $this->assertSame($expected, self::$sniff->indicies);
    }
