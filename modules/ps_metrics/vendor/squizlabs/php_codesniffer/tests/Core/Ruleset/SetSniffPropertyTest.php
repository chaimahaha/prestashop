...
final class SetSniffPropertyTest extends TestCase
{
    ...
    public function testSniffPropertiesGetSetWhenAllowed($name)
    {
        ...
        $properties = ['arbitrarystring' => 'arbitraryvalue', 'arbitraryarray' => ['mykey' => 'myvalue', 'otherkey' => 'othervalue']];
        // Set up the ruleset.
    ...
    {
        return ['Property allowed as explicitly declared' => ['AllowedAsDeclared'], 'Property allowed as sniff extends stdClass' => ['AllowedViaStdClass'], 'Property allowed as sniff has magic __set() method' => ['AllowedViaMagicMethod']];
    }
    ...
    public function testSetPropertyAppliesPropertyToMultipleSniffsInCategory()
    {
        ...
        $sniffClass = 'ps_metrics_module_v4_0_6\\PHP_CodeSniffer\\Standards\\PEAR\\Sniffs\\Functions\\FunctionCallSignatureSniff';
        ...
        $sniffClass = 'ps_metrics_module_v4_0_6\\PHP_CodeSniffer\\Standards\\PEAR\\Sniffs\\Functions\\FunctionDeclarationSniff';
        $this->assertArrayHasKey($sniffClass, $ruleset->sniffs, 'Sniff class ' . $sniffClass . ' not listed in registered sniffs');
        $sniffObject = $ruleset->sniffs[$sniffClass];
        $this->assertSame($expectedValue, $sniffObject->{$propertyName}, 'Property value not set to expected value for ' . $sniffClass);
        // Test that the property doesn't get set for the one sniff which doesn't support the property.
        $sniffClass = 'ps_metrics_module_v4_0_6\\PHP_CodeSniffer\\Standards\\PEAR\\Sniffs\\Functions\\ValidDefaultValueSniff';
        $this->assertArrayHasKey($sniffClass, $ruleset->sniffs, 'Sniff class ' . $sniffClass . ' not listed in registered sniffs');
    ...
    {
        $exceptionClass = 'ps_metrics_module_v4_0_6\\PHP_CodeSniffer\\Exceptions\\RuntimeException';
    ...
    public function testDirectCallWithNewArrayFormatSetsProperty()
    {
        ...
        $ruleset->setSniffProperty($sniffClass, $propertyName, ['scope' => 'sniff', 'value' => $propertyValue]);
        // Verify that the sniff has been registered.
        $this->assertGreaterThan(0, \count($ruleset->sniffCodes), 'No sniff codes registered');
        // Verify that our target sniff has been registered.
        $this->assertArrayHasKey($sniffCode, $ruleset->sniffCodes, 'Target sniff not registered');
    ...
    public function testDirectCallWithOldArrayFormatSetsProperty($propertyValue)
    {
        ...
        $ruleset = new Ruleset($config);
        $propertyName = 'arbitrarystring';
        @$ruleset->setSniffProperty($sniffClass, $propertyName, $propertyValue);
        // Verify that the sniff has been registered.
        $this->assertGreaterThan(0, \count($ruleset->sniffCodes), 'No sniff codes registered');
        // Verify that our target sniff has been registered.
        $this->assertArrayHasKey($sniffCode, $ruleset->sniffCodes, 'Target sniff not registered');
    ...
    {
        return ['Property value is not an array (boolean)' => ['propertyValue' => \false], 'Property value is not an array (string)' => ['propertyValue' => 'a string'], 'Property value is an empty array' => ['propertyValue' => []], 'Property value is an array without keys' => ['propertyValue' => ['value', \false]], 'Property value is an array without the "scope" or "value" keys' => ['propertyValue' => ['key1' => 'value', 'key2' => \false]], 'Property value is an array without the "scope" key' => ['propertyValue' => ['key1' => 'value', 'value' => \true]], 'Property value is an array without the "value" key' => ['propertyValue' => ['scope' => 'value', 'key2' => 1234]]];
    }
    ...
    {
        $exceptionClass = 'ps_metrics_module_v4_0_6\\PHPUnit\\Framework\\Error\\Deprecated';
