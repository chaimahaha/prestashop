...
final class TypedConstantsTest extends AbstractTokenizerTestCase
{
    ...
    public function testTernaryIsInlineThen()
    {
        ...
        $this->assertSame(T_INLINE_THEN, $tokens[$target]['code'], 'Token tokenized as ' . Tokens::tokenName($tokens[$target]['code']) . ', not T_INLINE_THEN (code)');
        $this->assertSame('T_INLINE_THEN', $tokens[$target]['type'], 'Token tokenized as ' . $tokens[$target]['type'] . ', not T_INLINE_THEN (type)');
    }
    ...
    public function testUntypedConstant($testMarker)
    {
        ...
        for ($i = $target + 1; $tokens[$i]['code'] !== T_EQUAL; $i++) {
            ...
            $this->assertSame(\T_STRING, $tokens[$i]['code'], 'Token tokenized as ' . Tokens::tokenName($tokens[$i]['code']) . ', not T_STRING (code)');
            $this->assertSame('T_STRING', $tokens[$i]['type'], 'Token tokenized as ' . $tokens[$i]['type'] . ', not T_STRING (type)');
        }
    }
    ...
    {
        return ['non OO constant (untyped)' => ['testMarker' => '/* testGlobalConstantCannotBeTyped */'], 'OO constant, final, untyped' => ['testMarker' => '/* testClassConstFinalUntyped */'], 'OO constant, public, untyped, with comment' => ['testMarker' => '/* testClassConstVisibilityUntyped */']];
    }
    ...
    public function testTypedConstant($testMarker, array $sequence)
    {
        ...
        for ($i = $target + 1; $tokens[$i]['code'] !== T_EQUAL; $i++) {
            ...
            $this->assertSame($sequence[$current], $tokens[$i]['code'], 'Token tokenized as ' . Tokens::tokenName($tokens[$i]['code']) . ', not ' . Tokens::tokenName($sequence[$current]) . ' (code)');
            ++$current;
        }
    }
    ...
    public static function dataNullableTypedConstant()
    {
        $data = [
            ...
            'global typed constant, invalid, ?int' => ['testMarker' => '/* testGlobalConstantTypedShouldStillBeHandled */', 'sequence' => [\T_STRING]],
            // OO constants.
            'nullable type: true' => ['testMarker' => '/* testTraitConstTypedNullableTrue */', 'sequence' => [T_TRUE]],
            'nullable type: false' => ['testMarker' => '/* testTraitConstTypedNullableFalse */', 'sequence' => [T_FALSE]],
            'nullable type: null' => ['testMarker' => '/* testTraitConstTypedNullableNull */', 'sequence' => [T_NULL]],
            'nullable type: bool' => ['testMarker' => '/* testTraitConstTypedNullableBool */', 'sequence' => [\T_STRING]],
            'nullable type: int' => ['testMarker' => '/* testTraitConstTypedNullableInt */', 'sequence' => [\T_STRING]],
            'nullable type: float' => ['testMarker' => '/* testTraitConstTypedNullableFloat */', 'sequence' => [\T_STRING]],
            'nullable type: string' => ['testMarker' => '/* testTraitConstTypedNullableString */', 'sequence' => [\T_STRING]],
            'nullable type: array' => ['testMarker' => '/* testTraitConstTypedNullableArray */', 'sequence' => [\T_STRING]],
            'nullable type: object' => ['testMarker' => '/* testTraitConstTypedNullableObject */', 'sequence' => [\T_STRING]],
            'nullable type: iterable' => ['testMarker' => '/* testTraitConstTypedNullableIterable */', 'sequence' => [\T_STRING]],
            'nullable type: mixed' => ['testMarker' => '/* testTraitConstTypedNullableMixed */', 'sequence' => [\T_STRING]],
            'nullable type: unqualified name' => ['testMarker' => '/* testTraitConstTypedNullableClassUnqualified */', 'sequence' => [\T_STRING]],
            'nullable type: fully qualified name' => ['testMarker' => '/* testTraitConstTypedNullableClassFullyQualified */', 'sequence' => [\T_NS_SEPARATOR, \T_STRING]],
            'nullable type: namespace relative name' => ['testMarker' => '/* testTraitConstTypedNullableClassNamespaceRelative */', 'sequence' => [\T_NAMESPACE, \T_NS_SEPARATOR, \T_STRING]],
            'nullable type: partially qualified name' => ['testMarker' => '/* testTraitConstTypedNullableClassPartiallyQualified */', 'sequence' => [\T_STRING, \T_NS_SEPARATOR, \T_STRING]],
            'nullable type: parent' => ['testMarker' => '/* testTraitConstTypedNullableParent */', 'sequence' => [T_PARENT]],
            'nullable type: self (only valid in enum)' => ['testMarker' => '/* testEnumConstTypedNullableSelf */', 'sequence' => [T_SELF]],
            'nullable type: static (only valid in enum)' => ['testMarker' => '/* testEnumConstTypedNullableStatic */', 'sequence' => [\T_STATIC]],
        ];
