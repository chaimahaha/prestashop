...
final class DefaultKeywordTest extends AbstractTokenizerTestCase
{
    ...
    public function testSwitchDefault($testMarker, $openerOffset, $closerOffset, $conditionStop = null, $testContent = 'default')
    {
        ...
        if ($opener + 1 !== $closer) {
            $end = $closer;
            if (isset($conditionStop) === \true) {
                $end = $conditionStop;
            }
            for ($i = $opener + 1; $i < $end; $i++) {
                $this->assertArrayHasKey($token, $tokens[$i]['conditions'], 'T_DEFAULT condition not added for token belonging to the T_DEFAULT structure');
            }
        }
    }
    ...
    {
        return ['simple_switch_default' => ['testMarker' => '/* testSimpleSwitchDefault */', 'openerOffset' => 1, 'closerOffset' => 4], 'simple_switch_default_with_curlies' => [
            ...
            'closerOffset' => 12,
            'conditionStop' => 6,
        ], 'switch_default_toplevel' => ['testMarker' => '/* testSwitchDefault */', 'openerOffset' => 1, 'closerOffset' => 43], 'switch_default_nested_in_match_case' => ['testMarker' => '/* testSwitchDefaultNestedInMatchCase */', 'openerOffset' => 1, 'closerOffset' => 20], 'switch_default_nested_in_match_default' => ['testMarker' => '/* testSwitchDefaultNestedInMatchDefault */', 'openerOffset' => 1, 'closerOffset' => 18]];
    }
